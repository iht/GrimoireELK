...
class CodeMetricsEnrich(Enrich):
    """
    Enrich the git output, producing file-level output with basic metrics.

    It produces an output similar to the Git backend, except that the
    granularity level is file.
    ...
    def get_field_date(self):
        """Get method for creation_date."""
        return "grimoire_creation_date"

    def get_field_unique_id(self):
        """Get method for Ocean uuid."""
        return "ocean-unique-id"

    def get_fields_uuid(self):
        """Get author and committer uuids."""
        return ["author_uuid", "committer_uuid"]

    def get_elastic_mappings(self):
        """Get mapping for Elastic."""
        mapping = """
        {
            "properties": {
               "message_analyzed": {
